...
class MFCC():
    ...
    def load_wav2(self,file_name,data_num,num):
        ...
        #print(len(x))
        #de-tamizumashisuru
        x = np.append(x , x * random.uniform(0.8, 1.2),axis = 0)
        x = np.append(x , x * random.uniform(0.8, 1.2),axis = 0)
    ...
    def fitdata(self):
        ...
        test_input_shape = [ -1,self.mel_len ,self.n_mels,1]#*3
        ...
        #CNNモデル
        model = Sequential()
        model.add(Conv2D(self.SPLICE_SIZE,(3,3), padding='same',
                         input_shape = data_num[1:],#(self.SPLICE_SIZE,self.n_mels*3),
                         activation='relu'))
        model.add(Conv2D(self.SPLICE_SIZE,(3,3), padding='same', activation='relu'))
        ...
        model.add(Dropout(0.25))
        """
        model.add(Conv2D(int(self.SPLICE_SIZE / 4), (3,3), padding='same', activation='relu'))
        ...
        #CNNモデルの保存
        with open(pkl_name+'_m.pkl','wb') as f:
            pickle.dump(model,f)
        print('finish')

    def cnn_predict(self,file_name):#予測
        with open(pkl_name + '_m.pkl','rb') as f:
            model = pickle.load(f)
        try:
            x, fs = sf.read('./'+ file_name)
        except Exception as e:
